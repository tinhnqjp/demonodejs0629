(function () {
  ...
  function AuthenticationController($scope, $state, UsersService, $location, $window, Authentication, PasswordValidator, Notification) {
    ...
    initData();
    function initData() {
      vm.dataRoles = $scope.getDataRoles();
      if (vm.authentication.user && vm.authentication.user.roles[0] === 'admin') {
    ...
    function onUserSignupSuccess(response) {
      ...
      Notification.success({ message: '<i class="glyphicon glyphicon-ok"></i> サインアップ成功!' });
      // And redirect to the previous or home page
      $state.go('admin.users.list');
    }

    function onUserSignupError(response) {
      Notification.error({ message: response.data.message, title: '<i class="glyphicon glyphicon-remove"></i> ログインエラー!', delay: 6000 });
    }

    function onUserSigninSuccess(response) {
      // If successful we assign the response to the global user model
      vm.authentication.user = response;
      Notification.info({ message: 'ようこそ！' + response.firstName });
      // And redirect to the previous or home page
